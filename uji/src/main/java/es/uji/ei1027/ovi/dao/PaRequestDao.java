...
public class PaRequestDao {
    ...
    public List<PaRequest> getRequestsAceptadasParaPappati(int idPapPati) {
        return jdbcTemplate.query(
                "SELECT * FROM pa_request WHERE id_pappati_asignado = ? AND status = 'Aceptado'",
                new PaRequestRowMapper(),
                idPapPati
        );
    }

    public void asignarAsistente(int idRequest, int idPapPati) {
        String sql = "UPDATE pa_request SET id_pappati_asignado = ?, status = 'Aceptado'::status_pa_request_enum WHERE id = ?";

        jdbcTemplate.update(sql, idPapPati, idRequest);
    }
}