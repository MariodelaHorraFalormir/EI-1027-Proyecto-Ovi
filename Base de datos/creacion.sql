...
CREATE TABLE persona (
    ...
    comunicacion      INTEGER DEFAULT 5,
    expresividad      INTEGER DEFAULT 5,
    caracter          INTEGER DEFAULT 5,
    naturaleza        INTEGER DEFAULT 5,
...
CREATE TABLE solicitud (
    ...
    -- Campos para revisión de solicitudes
    mensaje_revision         VARCHAR(500),
    fecha_ultima_revision    TIMESTAMP,
    numero_revisiones        INTEGER NOT NULL DEFAULT 0,
    ...
    CONSTRAINT ck_solicitud_fechas
        CHECK (
            ...
            OR (
                fecha_resolucion >= fecha_creacion 
                AND fecha_resolucion <= CURRENT_TIMESTAMP
            )
        ),
    ...
    CONSTRAINT ck_solicitud_estado_fecha
        CHECK (
            ...
            OR
            (estado IN ('Aprobada', 'Rechazada') AND fecha_resolucion IS NOT NULL)
        ),

    CONSTRAINT ck_solicitud_rechazo_motivo
        CHECK (
            estado <> 'Rechazada'
            OR motivo_resolucion IS NOT NULL
        ),

    CONSTRAINT ck_solicitud_revision_tecnico
        CHECK (
            estado = 'Pendiente'
            OR tecnico_revisor IS NOT NULL
        ),
    ...
    CONSTRAINT ck_solicitud_categoria_detalle
        CHECK (
            (
                categoria = 'Rol'
                AND detalle IN ('Pap_pati', 'Ovi_user')
            )
            OR
            (
                categoria = 'Proceso'
                AND detalle IN ('Pa_request', 'Asistencia_tecnica', 'Otro')
            )
        )
);